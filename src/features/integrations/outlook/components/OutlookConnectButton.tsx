...
export function OutlookConnectButton({
  ...
  useEffect(() => {
    ...
    const fetchStatus = async () => {
      try {
        const response = await fetch("/api/oauth/outlook/status", {
          method: "GET",
          credentials: "include",
          cache: "no-store",
        });
        ...
        if (!response.ok) {
          throw new Error("No se pudo recuperar el estado de Outlook");
        }
  ...
  const handleClick = useCallback(() => {
    ...
    const url = new URL("/api/oauth/outlook/start", window.location.origin);
