          ...
          <table className="w-full table-fixed text-sm">
            <thead className="sticky top-0 bg-slate-50">
              <tr className="text-left text-xs font-semibold text-slate-600">
                <th className="w-[10%] px-4 py-3 whitespace-nowrap">Fecha</th>
                <th className="w-[14%] px-4 py-3">Herramienta</th>
                <th className="w-[12%] px-4 py-3">Necesidad</th>
                <th className="w-[18%] px-4 py-3">Usuario</th>
                <th className="w-[18%] px-4 py-3">Empresa</th>
                <th className="w-[28%] px-4 py-3">Comentarios</th>
              </tr>
            </thead>
            <tbody>
              {(data?.solicitudes ?? []).map((row) => (
                <tr key={row.id} className="border-t border-slate-100">
                  <td className="w-[10%] whitespace-nowrap px-4 py-3">
                    <TruncateWithTooltip value={formatDate(row.created_at)} />
                  </td>
                  <td className="w-[14%] px-4 py-3"><TruncateWithTooltip value={row.herramienta} /></td>
                  ...
                  <td className="w-[18%] px-4 py-3"><TruncateWithTooltip value={`${row.first_name} ${row.last_name}`} /></td>
                  <td className="w-[18%] px-4 py-3"><TruncateWithTooltip value={row.user_businessname} /></td>
                  <td className="w-[28%] px-4 py-3"><TruncateWithTooltip value={row.comentarios ?? ''} /></td>
                </tr>
              ))}
            </tbody>
          </table>
