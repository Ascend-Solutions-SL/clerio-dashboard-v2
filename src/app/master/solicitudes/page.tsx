          ...
          <table className="w-full table-fixed text-sm">
            <thead className="sticky top-0 bg-slate-50">
              <tr className="text-left text-xs font-semibold text-slate-600">
                ...
                <th className="w-[180px] px-4 py-3">Herramienta</th>
                <th className="w-[140px] px-4 py-3">Necesidad</th>
                <th className="w-[260px] px-4 py-3">Usuario</th>
                <th className="w-[240px] px-4 py-3">Empresa</th>
                <th className="w-[420px] px-4 py-3">Comentarios</th>
              </tr>
            </thead>
            <tbody>
              {(data?.solicitudes ?? []).map((row) => (
                <tr key={row.id} className="border-t border-slate-100">
                  ...
                  <td className="w-[180px] truncate px-4 py-3">{row.herramienta}</td>
                  ...
                  <td className="w-[260px] truncate px-4 py-3">{row.first_name} {row.last_name}</td>
                  <td className="w-[240px] truncate px-4 py-3">{row.user_businessname}</td>
                  <td className="w-[420px] truncate px-4 py-3">{row.comentarios ?? ''}</td>
                </tr>
              ))}
            </tbody>
          </table>
